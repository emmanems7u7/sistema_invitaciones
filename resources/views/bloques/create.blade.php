...
@section('content')
                        ...
                        <div class="row g-2">
                            <div class="col-md-6">
                                ...
                                @if ($miniatura != null)
                                    ...
                                    <img src="{{ asset($miniatura->ruta) }}" alt="" style="width: 50%">
                                    <form action="{{ route('miniatura.destroy', $miniatura->id) }}" method="POST"
                                        style="display: inline;">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="btn btn-sm btn-warning"
                                            onclick="return confirm('¿Estás seguro de eliminar esta miniatura?')">
                                            Eliminar miniatura
                                        </button>
                                    </form>
                                @else
                                    <p class="text-danger">No hay miniatura asignada</p>
                                @endif
                                                ...
                                                <div class="card">
                                                    ...
                                                    <div id="bloque-{{ $bloque->id }}" class="card-body collapse hide">
                                                        ...
                                                        @if($bloque->tipo != 'ubicacion')
                                                            ...
                                                            <form action="{{ route('bloques.destroy', ['bloque' => $bloque]) }}"
                                                            ...
                                                            <div id="textsContainer_{{ $bloque->id }}" class="mt-4">
                                                                @include('bloques.textos_agregados')
                                                            </div>
                                                            <div id="mediaContainer_{{ $bloque->id }}">
                                                                @include('bloques.multimedia')
                                                            </div>
                                                        @else
                                                            <div class="row">
                                                                @include('bloques.ubicaciones')
        ...
        <div class="row mt-3">
            <div class="col-12">
                <div class="card shadow-sm">
                    ...
                    <div class="card-body">
                        <div class="row">
                            @include('bloques.contenido_cargado')
                        </div>
                    </div>
                </div>
            </div>
    ...
    <div class="modal fade" id="uploadImageModal" tabindex="-1" aria-labelledby="uploadImageModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <form id="imageUploadForm" method="POST" action="{{ route('imagenprev.store', $id) }}"
                enctype="multipart/form-data" class="modal-content">
                @csrf
                <div class="modal-header">
                    <h5 class="modal-title" id="uploadImageModalLabel">Subir Imagen con Previsualización</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Cerrar"></button>
                </div>
                <div class="modal-body">
                    <div class="mb-3">
                        <label for="imagenInput" class="form-label">Selecciona una imagen</label>
                        <input class="form-control" type="file" id="imagenInput" name="imagen" accept="image/*" required>
                    </div>
                    <div class="mb-3 text-center">
                        <img id="previewImagen" src="#" alt="Previsualización"
                            style="max-width: 100%; max-height: 250px; display: none; border-radius: 8px;" />
                    </div>
                </div>
                    ...
                    <form id="createColorForm" method="POST">
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control" id="tipo" name="tipo" required>
                                ...
                                <option value="fondo_inicio">Fondo Inicio</option>
                            </select>
                            <div class="invalid-feedback">Este campo es obligatorio.</div>
                        </div>
