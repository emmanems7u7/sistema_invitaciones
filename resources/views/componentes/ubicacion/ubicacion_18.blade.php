<style>
    ...
    .button-f {
        ...
        color: #181818;
        cursor: pointer;
        border: 1px solid var(--primary_borde);
        border-radius: 8px;
...
<div id="wedding" class="wedding-box pt-4">
    <div class="container">
        <div class="row justify-content-center">
            @foreach ($bloque['contenido'] as $index => $ubicacion)
                <div class="col-md-6  d-flex justify-content-center">
                    <div class="serviceBox text-center">
                        <!-- Comprobamos si tiene imagen o icono -->
                        @if ($ubicacion->imagen)
                            <div class="service-icon">
                                <img src="{{ asset('storage/' . $ubicacion->imagen) }}" alt="Imagen de actividad"
                                    style="max-width: 100%; height: auto;">
                            </div>
                        @elseif ($ubicacion->icono)
                            <div class="service-icon">
                                <!-- Mostramos el icono -->
                                <i class="{{ $ubicacion->icono }}"></i>
                            </div>
                        @endif
                        <h3 class="title">{{ $ubicacion->actividad }}</h3>
                        <h4>{{ $ubicacion->fecha }} a las {{ $ubicacion->hora_inicio }}</h4>
                        ...
                        <button class="button-f mt-2"
                            onclick="openMapModal('{{ $ubicacion->geolocalizacion }}', '{{ $ubicacion->actividad }}','{{ $ubicacion->direccion }}')">
                            Ver Ubicación
                            <div class="icon-1">
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <!-- Dirección como un card sobrepuesto -->
                <div class="card position-absolute top-0 start-50 translate-middle-x"
                    style="width: 90%; z-index: 10; max-width: 90%; background-color: rgba(255, 255, 255, 0.8);">
                    <div class="card-body text-center p-2">

                        <p id="direccionTexto" class="m-0">Cargando dirección...</p>
                ...
                <div class="mt-3 text-center">
                    <div class="mt-3 text-center">
                        <a id="googleMapsLink" href="#" target="_blank" class="btn btn-primary">
...
<!-- Incluir Leaflet Control Geolocate CSS -->
<link rel="stylesheet" href="https://unpkg.com/leaflet-control-geolocate/dist/L.Control.Geolocate.css" />

<!-- Incluir Leaflet Control Geolocate JS -->
<script src="https://unpkg.com/leaflet-control-geolocate/dist/L.Control.Geolocate.js"></script>

<script>

    let map;

    // Función para abrir el modal con la ubicación en el mapa
    function openMapModal(geolocalizacion, actividad, direccion) {

        document.getElementById('direccionTexto').textContent = direccion;
        let lat, lng;

        // Verificar si geolocalizacion es una cadena "lat,lng"
        if (typeof geolocalizacion === 'string') {
            const coords = geolocalizacion.split(',');
            lat = parseFloat(coords[0]);
            lng = parseFloat(coords[1]);
        }
        // Verificar si geolocalizacion es un objeto con lat y lng
        else if (typeof geolocalizacion === 'object' && geolocalizacion.lat && geolocalizacion.lng) {
            lat = geolocalizacion.lat;
            lng = geolocalizacion.lng;
        }
        ...
        if (isNaN(lat) || isNaN(lng)) {
            alert("Coordenadas inválidas. Asegúrate de que las coordenadas sean correctas.");
            return;
        }

        // Mostrar el modal antes de inicializar el mapa para que sea visible
        $('#mapModal').modal('show');

        // Esperar a que el modal se muestre completamente antes de cargar el mapa
        $('#mapModal').on('shown.bs.modal', function () {
            // Si el mapa ya está inicializado, lo eliminamos
            if (map) {
                map.remove();
            }
            ...
            L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
                attribution: '&copy; <a href="https://www.openstreetmap.org/copyright">Sis_inv</a> contributors',
            }).addTo(map);
            ...
            // Mover los controles de zoom a la parte inferior derecha
            map.zoomControl.setPosition('bottomright');

            // Agregar el botón de geolocalización
            L.control.locate({
                position: 'bottomright',
                follow: true, // Sigue la ubicación del usuario cuando se mueve
                setView: true, // Centra el mapa en la ubicación del usuario
                strings: {
                    title: "Ver mi ubicación"
                }
            }).addTo(map);

            // Corregir la visualización del mapa
            setTimeout(() => {
                map.invalidateSize();
            }, 300);
        });
